...
class VK
{
     ...
     * @return  string
     */
    public function request($url, $method = 'GET', $postfields = array(), $repetition = 0) {
        ...
        if(isset($rr['error']['error_code']) AND trim($rr['error']['error_code']) == 6/* AND $rr['error']['error_msg'] == 'Too many requests per second'*/ AND $repetition < 3) {
            sleep(1);
            $r = $this->request($url, $method, $postfields, $repetition+1);
